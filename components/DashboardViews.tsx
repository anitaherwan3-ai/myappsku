...
import { 
    ...
    Wind, HeartPulse, UserCheck, Clock, Microscope, FileSpreadsheet, MapPin
} from 'lucide-react';
...
// KOMPONEN RESUME PASIEN FORMAL (KOP SURAT RESMI)
export const PatientResumeContent = ({ patient }: { patient: Patient }) => {
    ...
    return (
        <div className="bg-white p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-8">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                <p className="text-[8px] font-black text-slate-400 uppercase">Status BMI</p>
                ...
                <div className="space-y-8">
                    {patient.category === 'Berobat' ? (
                        <section>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="p-4 bg-slate-900 rounded-2xl text-white">
                                    <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Diagnosis Utama (ICD-10)</p>
                                    <p className="text-xs font-black uppercase tracking-tight">[{patient.diagnosisCode || 'NON-ICD'}] {patient.diagnosisName || '-'}</p>
                                </div>
                        ...
                        <section>
                            <span className={sectionTitle}>Pemeriksaan Fisik & Penunjang (MCU)</span>
                            <div className="grid grid-cols-2 gap-x-8 gap-y-2">
                                {dataRow("Visus R/L", `${patient.visusOD}/${patient.visusOS}`)}
                                {dataRow("Buta Warna", patient.colorBlind)}
                                {dataRow("Telinga R/L", `${patient.rightEar}/${patient.leftEar}`)}
                                {dataRow("Hidung / Gigi", `${patient.nose} / ${patient.teeth}`)}
                            </div>
                            
                            <div className="mt-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Hasil Lab & Penunjang</p>
                                <p className="text-[10px] font-bold text-slate-600 leading-tight mb-2">Rontgen: {patient.rontgen} • EKG: {patient.ekg}</p>
                                <p className="text-[11px] font-bold text-slate-800 leading-relaxed">{patient.laboratory || '-'}</p>
                            </div>
                    ...
                    <section>
                        <span className={sectionTitle}>Hasil Akhir & Triage</span>
            ...
            <div className="flex justify-between mt-20 px-8 print:flex hidden">
                <div className="text-center">
                    <p className="text-[10px] font-bold uppercase mb-20 italic text-slate-500">Pasien / Keluarga,</p>
                    <div className="w-48 border-t border-slate-900 pt-1 mx-auto font-black text-xs">( ____________________ )</div>
                </div>
                <div className="text-center">
                    <p className="text-[10px] font-bold uppercase mb-20 italic text-slate-500">Petugas Medis PCC,</p>
                    <div className="w-48 border-t border-slate-900 pt-1 mx-auto font-black text-xs uppercase">{patient.lastModifiedBy || 'Officer PCC'}</div>
                </div>
            </div>
        </div>
    );
...
// MODAL DETAIL PASIEN (SINGLE RESUME)
const PatientDetailModal = ({ patient, onClose }: { patient: Patient, onClose: () => void }) => {
    return (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-4 print:p-0 print:static print:bg-white">
            <div className="bg-white rounded-[3rem] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-card print:shadow-none print:rounded-none print:max-w-none print:max-h-none print:static">
                <div className="p-8 border-b flex justify-between items-center bg-slate-50/50 no-print">
                    ...
                    <div className="flex gap-4 modal-header-buttons">
                        <button onClick={() => window.print()} className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 shadow-lg hover:bg-slate-800 transition-all"><Printer size={16}/> Cetak Resume</button>
                ...
                <div id="printable-resume" className="flex-1 overflow-y-auto p-10 custom-scrollbar bg-white print:p-0 print:overflow-visible">
                    <PatientResumeContent patient={patient} />
                </div>
            </div>
        </div>
    );
...
export const ManagePatients = ({ mode }: { mode: 'edit' | 'add' | 'record' }) => {
    ...
    return (
        <div className="animate-fade-in space-y-8">
            ...
            {/* PRINT-ONLY SECTION (REPORT) */}
            <div className="hidden print:block">
                <div className="print-header-letterhead">
                    <h1>LAPORAN DATA {mode === 'record' ? 'REKAM MEDIS' : 'REGISTRY'} PASIEN</h1>
                    <h2>PROVINCE COMMAND CENTER (PCC) SUMATERA SELATAN</h2>
                    <p>Sistem War Room Kesehatan - Monitoring Pelayanan Operasional</p>
                </div>
                
                {mode === 'record' ? (
                    /* Rekam Medis: Print all resumes sequentially */
                    <div className="space-y-12">
                        {filtered.map(p => (
                            <div key={p.id} className="bulk-resume-item">
                                <PatientResumeContent patient={p} />
                            </div>
                        ))}
                    </div>
                ) : (
                    /* Registry: Print Summary Table */
                    <table className="print-summary-table">
                        <thead>
                            <tr>
                                ...
                                <th>NIK</th>
                                <th>Kegiatan</th>
                                {filtered.some(p => p.category === 'Berobat') ? (
                                    <><th>Diagnosa</th></>
                                ) : null}
                                {filtered.some(p => p.category === 'MCU') ? (
                                    <><th>Gender/Usia</th><th>Kesimpulan Akhir</th></>
                                ) : null}
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.map((p, idx) => (
                                <tr key={p.id}>
                                    ...
                                    <td className="font-mono">{p.identityNo}</td>
                                    <td>{activities.find(a => a.id === p.activityId)?.name}</td>
                                    {filtered.some(px => px.category === 'Berobat') ? (
                                        <td>{p.category === 'Berobat' ? p.diagnosisName : '-'}</td>
                                    ) : null}
                                    {filtered.some(px => px.category === 'MCU') ? (
                                        <>
                                            <td>{p.gender}/{p.age} Thn</td>
                                            <td>{p.category === 'MCU' ? p.mcuConclusion : '-'}</td>
                                        </>
                                    ) : null}
                                </tr>
                            ))}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 no-print">
                ...
                <div className="flex gap-4">
                    <button onClick={handlePrintBulk} className="bg-white border px-6 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 shadow-sm hover:shadow-md transition-all active:scale-95"><Printer size={18}/> Cetak Laporan</button>
            ...
            <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden overflow-x-auto no-print dashboard-table-container">
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div className="flex justify-between items-center no-print">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter">Master Kegiatan Operasional</h2>
                <button 
                    onClick={() => {
                    ...
                    className="bg-primary text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all"
                >
...
export const ManageOfficers = () => {
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div className="flex justify-between items-center no-print">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter">Manajemen Petugas PCC</h2>
                <button 
                    onClick={() => {
                        const name = prompt("Nama Petugas Baru:");
                        const email = prompt("Email Petugas:");
                        if (name && email) {
                            addOfficer({
                                id: crypto.randomUUID(),
                                name,
                                email,
                                teamId: 'PCC-' + Math.floor(100 + Math.random() * 900),
                                role: 'petugas'
                            });
                        }
                    }}
                    ...
                    <tbody>
                        {officers.map(o => (
                            <tr key={o.id} className={tableRowClass}>
                                ...
                                <td className={tableCellClass}><span className="font-mono uppercase text-[10px]">{o.teamId}</span></td>
                                <td className={`${tableCellClass} no-print`}>
            ...
            <div className="flex justify-between items-center no-print">
                ...
                <button 
                    onClick={() => {
                        ...
                        const imageUrl = prompt("URL Gambar Berita:");
                        if (title && imageUrl) {
                            addNews({
                                id: crypto.randomUUID(),
                                title,
                                ...
                                imageUrl,
                                content: 'Isi konten berita yang baru ditambahkan...'
                            });
                        }
                    }}
                    className="bg-primary text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all"
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (evt) => {
            ...
            const formatted = data.map(item => ({
                code: item.Code || item.code || item.KODE || item.kode,
                name: item.Name || item.name || item.NAMA || item.nama || item.Description
            })).filter(i => i.code && i.name);

            if (formatted.length > 0) {
                addICD10(formatted);
                alert(`Berhasil mengimpor ${formatted.length} kode ICD-10.`);
            }
        };
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 no-print">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter leading-none">Library Master ICD-10</h2>
                <div className="flex flex-wrap gap-3">
                    <input type="file" ref={fileInputRef} onChange={handleFileUpload} accept=".xlsx, .xls, .csv" className="hidden" />
                    <button 
                        onClick={() => fileInputRef.current?.click()}
                        className="bg-emerald-50 text-emerald-600 px-6 py-4 rounded-2xl font-black text-[10px] uppercase flex items-center gap-3 border border-emerald-100 hover:bg-emerald-600 hover:text-white transition-all"
                    >
                        <FileSpreadsheet size={18}/> Impor XLSX/CSV
                    </button>
                    <button 
                        onClick={() => {
                            const code = prompt("Kode ICD-10:");
                            const name = prompt("Nama Diagnosa:");
                            if (code && name) {
                                addICD10([{ code, name }]);
                            }
                        }}
...
export const ManageCarousel = () => {
    ...
    return (
        <div className="animate-fade-in space-y-8">
            <div className="flex justify-between items-center no-print">
                <h2 className="text-3xl font-black uppercase italic tracking-tighter">Konfigurasi Slide Portal</h2>
                <button 
                    onClick={() => {
                        const title = prompt("Judul Slide:");
                        const imageUrl = prompt("URL Gambar Slide:");
                        if (title && imageUrl) {
                            addCarousel({
                                id: crypto.randomUUID(),
                                title,
                                subtitle: 'Subtitle slide portal...',
                                imageUrl
                            });
                        }
                    }}
